...
# Gemini API Initialization - Optimized for Gemini 2.0
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "")
if GEMINI_API_KEY:
    genai.configure(api_key=GEMINI_API_KEY)
    # Using gemini-2.0-flash (confirmed available via diagnostics)
    try:
...
async def create_test(request: TestCreateRequest):
    ...
    try:
        ...
        # Call Gemini to generate questions in a SINGLE request (Replica Match)
        try:
            if not gemini_model:
                raise ValueError("Gemini API key not configured")
            
            # Using the exact same configuration as the ffffff replica
            replica_model = genai.GenerativeModel(
                'gemini-2.5-flash',
            ...
            response = replica_model.generate_content(prompt)
            ...
            error_msg = str(e)
            logger.error(f"Error generating questions via Gemini (Replica Mode): {error_msg}")
            
            # Quota Check or Auth Failure - Fallback with clear instructions
            fallback_q = Question(
                test_id=new_test.id,
                question=f"The Gemini AI service is currently hit with a limit or mismatch (Error: {error_msg[:60]}...). Please try using 'Import from Excel' as a replica fallback.",
                option_a="I understand",
                option_b="I'll try later",
                option_c="I'll use Excel",
                option_d="Notify Admin",
                correct_answer="I understand"
            )
            db.add(fallback_q)
            db.commit()
            return {"message": "Test generated, but AI hit a replica mismatch. Check test questions.", "test_id": new_test.id, "api_error": True}
...
async def generate_student_analytics(student_roll: str):
    ...
    try:
        ...
        if not recommendations and performance_level == "Strong":
    ...
    try:
        ...
        for s in students:
            ...
            results.append({
                ...
                "year": s.year,
                "section": s.section,
                "performance": profile.performance_category if profile else "No Data"
            })
        return results
    finally:
        db.close()
...
async def update_student(roll_number: str, data: dict):
    ...
    try:
        ...
        if "email" in data: student.email = data.get("email") or ""
...
async def get_all_subjects():
    ...
    db: Session = SessionLocal()
    try:
        # Get unique subjects from tests or student performance records
        test_subjects = db.query(Test.subject).distinct().all()
        perf_subjects = db.query(StudentPerformance.subject).distinct().all()
        all_subjects = list(set([s[0] for s in test_subjects] + [s[0] for s in perf_subjects]))
        return [{"id": i, "name": s} for i, s in enumerate(all_subjects) if s]
    finally:
        db.close()
...
async def bulk_register_students(file: UploadFile = File(...)):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Auto-add section to sections table if it doesn't exist
            if section_name and not db.query(Section).filter(Section.name == section_name).first():
                new_sec = Section(name=section_name, branch=str(row.get("branch", "CSE")), year=str(row.get("year", "1st Year")))
                db.add(new_sec)
        ...
        db.add(log)
        db.commit()
        return {"message": f"Successfully imported {count} students. Skipped {skipped} existing records."}
    except Exception as e:
...
async def generate_exam_variations(request: VariationRequest):
    ...
    try:
        ...
        logger.error(f"Variation Generation Error (Replica Mode): {e}")
        raise HTTPException(status_code=500, detail=str(e))
